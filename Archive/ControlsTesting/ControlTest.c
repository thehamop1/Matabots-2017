...
#pragma config(Motor,  port1,           testMotor,     tmotorVex393TurboSpeed_HBridge, openLoop)
...
#define PROPORTIONAL_GAIN 1.00 //PID controller proportinal
#define INTEGRAL_CONST 0.0//896.9 //PID controller integral
#define SAMPLE_PERIOD 100  //Sample period for digital control

#define PI_RANGE 20//5 	//we are switching between proportional controller and PI controller.
...
void ReadSpeed(MOTOR_PI* motorA, int time){
  ...
  int encoder = SensorValue[(*motorA).sensorname];

  (*motorA).speedRead = (float)((58.138*(encoder - (*motorA).lastEncoderRead)))/((float)time); //that is not a unit. You should multiply for a constant. I will leave this way by now.
...
task main()
{
	initMotor(&testM, testMotor, driveEncoder);

	testM.speedSet = 50;
	while(true)
	{
	  writeDebugStreamLine("%4f , %4f , %4f", testM.speedSet, testM.speedRead, (testM.speedSet - testM.speedRead));
  	ControlFunction(&testM);
	  wait1Msec(SAMPLE_PERIOD/4);
	}
}
